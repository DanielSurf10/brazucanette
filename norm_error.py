...
try:
	with open(os.path.expanduser("~/.local/share/norminette/norm_error.json"), 'r') as arq:
		new = json.load(arq)
		errors.update(new)
except:
	...
...
class NormError:
	def __init__(self, errno, line, col=None):
		...
		msg = errors.get(self.errno, 'ERROR NOT FOUND')
		if (isinstance(msg, str)):
			self.error_msg = f"{msg}"
		else:
			random_msg = random.randint(0, len(msg) - 1)
			self.error_msg = f"{msg[random_msg]}"
...
class NormWarning:
	def __init__(self, errno, line, col=None):
		...
		msg = errors.get(self.errno, 'WARNING NOT FOUND')
		if (isinstance(msg, str)):
			self.error_msg = f"{msg}"
		else:
			random_msg = random.randint(0, len(msg) - 1)
